 ...
 * From SDM volume 3, section 24.6.1
 */
 ...
 * From SDM volume 3, section 24.6.2, Table 24-6
 */
struct capability_info procbased_primary[22] =
{
	{ 2, "nterrupt-window exiting" },
...
/*
 * Primary processor-based capabilities
 * From SDM volume 3, section 24.6.2, Table 24-7
 */
struct capability_info procbased_secondary[28] =
{
 ...
 * From SDM volume 3, section 24.6.2, Table 24-8
*/
 ...
 * VM-Exit Controls capabilities
  * From SDM volume 3, section 24.7.1, Table 24-13. Definitions of Primary VM-Exit Controls
 */
struct capability_info vm_exit_controls[17] =
{	
 ...
 *  VM-Entry Controls capabilities
  * From SDM volume 3, section 24.8.1, Table 24-15. Definitions of VM-Entry Controls
 */
...
{
	uint32_t lo, hi;
	uint32_t act_secondary_controls, act_tertiary_controls;
	...
	act_secondary_controls = (hi & (1 << 31)) ? 1 : 0;

	if(act_secondary_controls)
	{
		rdmsr(IA32_VMX_PROCBASED_CTLS2, lo, hi);
		pr_info("Secondary Processor-Based Controls MSR: 0x%llx\n",
			(uint64_t)(lo | (uint64_t)hi << 32));
		report_capability(procbased_secondary, 28, lo, hi);
	}
	else
		pr_info("Cannot Set Secondary Processor-Based Controls");


	/* Tertiary Processor-Based controls */
	act_tertiary_controls = (hi & (1 << 17)) ? 1 : 0;
	
	if(act_tertiary_controls)
	{
		...
		pr_info("Cannot Set Tertiary Processor-Based Controls");
	...
	pr_info("Exit Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(vm_exit_controls, 17, lo, hi);
...
init_module(void)
{
	printk(KERN_INFO "CMPE 283 Assignment 1 Module Start -- SSK \n");
...
cleanup_module(void)
{
	printk(KERN_INFO "CMPE 283 Assignment 1 Module Exits -- SSK \n");
}
